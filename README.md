...
**ALIO 데이터 준비 (필수)** — 자기 PC 에서 ALIO 도구를 쓰려면 데이터가 있어야 합니다. 두 방법 중 하나로 준비하세요.

#### (방법 1) 운영자 mirror 사용 (5-15분, 추천)

미리 수집해둔 데이터를 다운로드. 외부 도구 설치 불필요. 약 200MB 압축본 → 풀면 1.27GB.
...
ALIO 공시에서 344개 공공기관 35,000건 규정을 직접 sync. 최신 데이터를 본인이 통제 가능. HWP/HWPX/PDF/XLSX 변환을 위해 OS별 외부 도구 설치 권장 (없어도 일부 케이스만 `parseError` 로 남고 동작):
...
**Windows:** Node.js 만 있으면 sync 자체는 동작. 외부 도구가 없으면 스캔 PDF·HWP 3.0 같은 특수 케이스가 `parseError` 로 표시될 수 있음.
...
수집된 데이터는 `data/alio/` 에 저장 (약 1.27GB). 기관별 디렉터리 (`C0xxx`) + manifest.json + regulations 마크다운.
