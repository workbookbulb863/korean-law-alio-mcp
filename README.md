...
- **공공기관 규정과 법제처 법령을 잇는 연계 도구 3개**
  - `analyze_regulation_delegation` — 공공기관 규정 본문에서 인용된 상위 법령 자동 추출 + 법제처 `search_law` 연계로 법령 식별자(MST/lawId) 첨부
  - `find_regulations_by_upper_law` — 법제처 법령을 입력하면, 그 법령을 근거로 삼는 공공기관 규정을 역검색
  - `parse_alio_article_links` — 단일 규정 안에서 조문끼리 서로 어떻게 인용·참조하는지 그래프로 분석
...
→ `analyze_regulation_delegation` 한 번으로 (실제 결과 예 — KISA 인사규정):
...
- 35,000건 공공기관 규정 본문에서 "근로기준법" 인용 위치 역검색
- 매칭된 지침의 인용 컨텍스트 (어느 조문이 어떻게 인용됐는지) + 기관별 그룹 표시
- 사용자가 그 결과를 보고 자기 기관 지침이 상위법을 적절히 따르고 있는지 검토 가능
